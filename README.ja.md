...
（native 版では bubblewrap サンドボックス上の rootfs）で、永続ホームと git 作業コピーを
...
- 本配布物のイメージと rootfs は **lean 構成**（エージェント CLI を焼き込まない
  ...
  利用者ごとにそれぞれの公式配布元から検証済みのピン版を取得します。これは、
  プロプライエタリな CLI を配布物に載せない（＝再配布しない）ための、意図的な
  構成です。
- 一方、自分でビルドする **全焼き込み Docker イメージ**（既定の
  `BAKE_AGENT_CLIS=1`）には、これらエージェント CLI のバイナリ本体がイメージに
  含まれます。**自組織内で使う分には問題ありませんが、そのイメージは再配布しないで
  ください**（公開レジストリへの push、`docker save` で固めたイメージ tar の第三者への
  配布 など）——プロプライエタリな CLI を再配布することになるためです。第三者へ
  イメージを渡す必要がある場合は、lean 構成（`BAKE_AGENT_CLIS=0`）でビルドして
  ください（本配布物のイメージ・rootfs は、まさにこの lean 構成でビルドしたものです）。
